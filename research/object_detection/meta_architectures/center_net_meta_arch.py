...
class CenterNetFeatureExtractor(tf.keras.Model):
    ...
    def __init__(self, name=None, channel_means=(0., 0., 0.),
        ...
        if channel_means is None or len(
                channel_means) == 0:  # pylint:disable=g-explicit-length-test
            channel_means = [0., 0., 0.]

        if channel_stds is None or len(
                channel_stds) == 0:  # pylint:disable=g-explicit-length-test
            channel_stds = [1., 1., 1.]
...
def top_k_feature_map_locations(feature_map, max_pool_kernel_size=3, k=100,
    ...
    if per_channel:
        ...
        scores, peak_flat_indices = tf.math.top_k(feature_map_peaks_transposed,
                                                  k=k)
...
def prediction_tensors_to_keypoint_candidates(
    ...
    selected_offsets = tf.gather(offsets_flat, peak_spatial_indices,
                                 batch_dims=1)
...
def _gather_candidates_at_indices(keypoint_candidates, keypoint_scores,
    ...
    gathered_keypoint_candidates = tf.transpose(
        nearby_candidate_coords_tranposed,
        [0, 2, 1, 3])
...
class CenterNetMetaArch(model.DetectionModel):
    ...
    def batched_prediction_tensor_names(self):
        if not self._batched_prediction_tensor_names:
            raise RuntimeError(
                'Must call predict() method to get batched prediction '
                'tensor names.')
    ...
    def _compute_object_center_loss(self, input_height, input_width,
        ...
        if self._center_params.use_only_known_classes:
            ...
            batch_labeled_classes = tf.reshape(
                ...
                [batch_labeled_classes_shape[0], 1,
                 batch_labeled_classes_shape[-1]])
    ...
    def _compute_box_scale_and_offset_loss(self, input_height, input_width,
        ...
        for scale_pred, offset_pred in zip(scale_predictions,
                                           offset_predictions):
    ...
    def _compute_kp_heatmap_loss(self, input_height, input_width, task_name,
        ...
        """
        gt_keypoints_list = self.groundtruth_lists(
        ...
        flattened_heapmap_targets = _flatten_spatial_dimensions(
            keypoint_heatmap)
    ...
    def _compute_kp_offset_loss(self, input_height, input_width, task_name,
        ...
        for prediction in offset_predictions:
            batch_size, out_height, out_width, channels = _get_shape(prediction,
                                                                     4)
            if channels > 2:
                prediction = tf.reshape(
                    prediction,
                    shape=[batch_size, out_height, out_width, -1, 2])
    ...
    def _compute_densepose_losses(self, input_height, input_width,
        ...
        dp_heatmap_loss, dp_regression_loss = (
            self._compute_densepose_part_and_coordinate_losses(
                ...
                surface_coord_predictions=prediction_dict[
    ...
    def _compute_densepose_part_and_coordinate_losses(
        ...
        num_valid_points = tf.cast(tf.math.maximum(num_valid_points, 1),
                                   tf.float32)
        for part_pred, surface_coord_pred in zip(part_predictions,
            ...
            if self._densepose_params.upsample_to_input_res:
                part_pred = tf.keras.layers.UpSampling2D(
                    self._stride,
                    interpolation=self._densepose_params.upsample_method)(
                    part_pred)
    ...
    def loss(self, prediction_dict, true_image_shapes, scope=None):
        ...
        if self._od_params is not None:
            ...
            for key in od_losses:
                od_losses[key] = od_losses[
                                     key] * self._od_params.task_loss_weight
        ...
        if self._mask_params is not None:
            ...
            for key in seg_losses:
                seg_losses[key] = seg_losses[
                                      key] * self._mask_params.task_loss_weight
        ...
        if self._densepose_params is not None:
            ...
            for key in densepose_losses:
                densepose_losses[key] = (
                        densepose_losses[
                            key] * self._densepose_params.task_loss_weight)
    ...
    def postprocess(self, prediction_dict, true_image_shapes, **params):
        ...
        if self._mask_params:
            ...
            if self._densepose_params:
                ...
                prediction_dict[DENSEPOSE_REGRESSION][-1]
        ...
        for ex_ind in range(batch_size):
            ...
            for task_name, kp_params in self._kp_params_dict.items():
                ...
                def false_fn():
                    ...
                    return (
                    tf.zeros([1, 0, total_num_keypoints, 2], dtype=tf.float32),
                    tf.zeros([1, 0, total_num_keypoints], dtype=tf.float32))

                true_fn = functools.partial(
                    true_fn, keypoint_heatmap, keypoint_offsets,
                    keypoint_regression,
            ...
            kpt_coords_for_example = tf.concat(kpt_coords_for_class_list,
                                               axis=1)
            kpt_scores_for_example = tf.concat(kpt_scores_for_class_list,
                                               axis=1)
            ...
            if tf.size(instance_inds_for_example) > 0:
                ...
                kpt_coords_for_example_all_det = tf.zeros(
                    [1, max_detections, total_num_keypoints, 2],
                    dtype=tf.float32)
    ...
    def _postprocess_keypoints_for_class_and_image(
            self, keypoint_heatmap, keypoint_offsets, keypoint_regression,
            classes,
        ...
        regressed_keypoints_for_objects = regressed_keypoints_at_object_centers(
            keypoint_regression, y_indices_for_kpt_class,
        ...
        refined_keypoints, refined_scores = refine_keypoints(
            regressed_keypoints_for_objects, keypoint_candidates,
            keypoint_scores,
